...
(function () {
  ...
  MobroSDK.init().then(() => {
    ...
    MobroSDK.addChannelListener('general_processor_usage', data => {
	  ...
	  charts.cpuUsage.chart.data.datasets[0].data.push(parseInt(data.payload.value))
      charts.cpuUsage.chart.data.datasets[0].data.shift();
      charts.cpuUsage.chart.config.data.labels.push(+ new Date())
      charts.cpuUsage.chart.config.data.labels.shift();
      charts.cpuUsage.chart.update();
    });
    ...
    MobroSDK.addChannelListener('general_graphics_usage', data => {
	  ...
	  charts.gpuUsage.chart.data.datasets[0].data.push(parseInt(data.payload.value))
      charts.gpuUsage.chart.data.datasets[0].data.shift();
      charts.gpuUsage.chart.config.data.labels.push(+ new Date())
      charts.gpuUsage.chart.config.data.labels.shift();
      charts.gpuUsage.chart.update();
    });
	MobroSDK.addChannelListener('general_memory_usage', (data) => {
		charts.ramUsage.chart.data.datasets[0].data.push(parseInt(data.payload.value))
		charts.ramUsage.chart.data.datasets[0].data.shift();
		charts.ramUsage.chart.config.data.labels.push(+ new Date())
		charts.ramUsage.chart.config.data.labels.shift();
    ...
    MobroSDK.addChannelListener('general_memory_used', data => {
      memoryData.innerHTML = convert(parseFloat(data.payload.value)).from(data.payload.unit).to('GB').toFixed(2) + 'GB';
    });
	...
	MobroSDK.addChannelListener("theme_vram_percentage", data => {
      if (data.payload) {
        charts.vramUsage.chart.data.datasets[0].data.push(parseInt(data.payload.value))
		charts.vramUsage.chart.data.datasets[0].data.shift();
		charts.vramUsage.chart.config.data.labels.push(+ new Date())
		charts.vramUsage.chart.config.data.labels.shift();
		charts.vramUsage.chart.update();
      } else {
        charts.vramUsage.chart.data.datasets[0].data.push(parseInt('0'))
		charts.vramUsage.chart.data.datasets[0].data.shift();
		charts.vramUsage.chart.config.data.labels.push(+ new Date())
		charts.vramUsage.chart.config.data.labels.shift();
		charts.vramUsage.chart.update();
...
function initCharts() {
  return {
	...
	"gpuUsage": createBar(document.getElementById("gpu-chart-bar")),
	"cpuUsage": createBar(document.getElementById("cpu-chart-bar")),
	"vramUsage": createBar(document.getElementById("vram-chart-bar")),
	"ramUsage": createBar(document.getElementById("ram-chart-bar"))
  };
}
...
function configureChartJS() {
  ...
  Chart.defaults.global.defaultFontFamily = "unispaceregular";
